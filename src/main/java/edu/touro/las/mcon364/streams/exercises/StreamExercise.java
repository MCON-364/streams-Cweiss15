...
public class StreamExercise {
    ...
    public double calculateAverage(String studentName) {
        return gradebook.getOrDefault(studentName, new ArrayList<>()).stream().mapToInt(Integer::intValue).average().orElse(0.0);
    }
    ...
    public int findHighestGrade() {
        return gradebook.values().stream().flatMap(List::stream).max(Integer::compareTo).get();
    }
    ...
    public int findLowestGrade() {
        return gradebook.values().stream().flatMap(List::stream).min(Integer::compareTo).get();
    }
    ...
    public List<String> getPassingStudents(double threshold) {
        return gradebook.keySet().stream().filter(x -> calculateAverage(x)>=threshold).toList();
    }
    ...
    public Map<String, List<String>> groupByPerformance() {
        Map<String, List<String>> groups = gradebook.entrySet().stream().collect(Collectors.groupingBy(
                student -> {
                    double avg = calculateAverage(student.getKey());
                    if (avg>=90) return "A";
