...
public class StreamHomework {
    ...
    public long getOrderCount(OrderStatus status) {
        return customerOrders.stream().filter(order -> order.status==status)
                .count();
    }
    ...
    public Set<Product> getUniqueProducts() {
        return customerOrders.stream().flatMap(order -> order.items.stream())
                .map(type -> type.product).collect(Collectors.toSet());
    }
    ...
    public double getAverageOrderValue() {
        return customerOrders.stream().filter(order -> order.status()==OrderStatus.DELIVERED)
                .mapToDouble(order -> order.getTotal()).average().orElse(0.0);
    }
        ...
        return customerOrders.stream().filter(order -> order.status == OrderStatus.DELIVERED)
                .collect(Collectors.groupingBy(order -> order.customerId,
                        Collectors.summingDouble(order -> order.items.stream()
                                .mapToDouble(item -> item.quantity*item.product.price).sum())));
    }
    ...
    public Map<String, Long> getCustomerOrderCounts() {
        return customerOrders.stream().collect(Collectors.groupingBy(order -> order.customerId, Collectors.counting()));
    }
    ...
    public List<String> getCustomersWithMultipleOrders() {
        return getCustomerOrderCounts().entrySet().stream().filter(orders -> orders.getValue()>1).map(Map.Entry::getKey).toList();
    }
    ...
    public Map<String, Double> getRevenueByCategory() {
        return customerOrders.stream().filter(order -> order.status==OrderStatus.DELIVERED)
                .flatMap(order -> order.items.stream()).collect(Collectors.groupingBy(item -> item.product.category,
    ...
    public List<Product> getTopSellingProducts(int n) {
        return customerOrders.stream().flatMap(order -> order.items.stream())
                .collect(Collectors.groupingBy(item -> item.product, Collectors.summingInt(item -> item.quantity)))
                        .entrySet().stream()
    ...
    public Map<String, Integer> getProductQuantitySold() {
        return customerOrders.stream().flatMap(order -> order.items.stream())
                .collect(Collectors.groupingBy(item -> item.product.id, Collectors.summingInt(item -> item.quantity)));
    }
        ...
        return customerOrders.stream()
                ...
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.teeing(
                                ...
                                (totalRevenue, totalQuantity) -> new CategorySummary(totalRevenue, totalQuantity)
                        )
                ));
    }
